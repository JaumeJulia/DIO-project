...
tabla1<- modelo_datos %>% group_by(genre) %>% 
  summarise(
            ...
            Min_num_of_tracks=
              min(num_of_tracks,na.rm = TRUE))
knitr::kable(tabla1)
#barplot(t(as.matrix(as.data.frame(tabla1))), beside=TRUE)
```
...
tabla3<- modelo_datos %>% group_by(genre) %>% 
  summarise(
            ...
            Min_Rating_RS=
              min(rolling_stone_critic,na.rm = TRUE))
knitr::kable(tabla3)
#barplot(t(as.matrix(tabla3)), beside=TRUE)
```
...
tabla4<- modelo_datos %>% group_by(genre) %>% 
  summarise(
            ...
            Min_mtv_critic=
              min(mtv_critic,na.rm = TRUE))
knitr::kable(tabla4)
#barplot(t(as.matrix(tabla4)), beside=TRUE)
```
...
tabla5<- modelo_datos %>% group_by(genre) %>% 
  summarise(
            ...
            Min_maniac_critic=
              min(music_maniac_critic,na.rm = TRUE))
knitr::kable(tabla5)
#barplot(t(as.matrix(tabla5)), beside=TRUE)
```
...
## -Rating revistas con género
## -Número de tracks por género
## -Comparación de los ratings entre revistas

## -Rating revistas con número de tracks
```{r}
...
```
## -Comparacion de los ratings entre revistas
```{r}
...
```
##Rating revistas con número de ventas
```{r}
...
```{r}
glimpse(music3$rolling_stone_critic)
ventas10=cut(music3$num_of_sales,breaks=10)
table(ventas10,music3$rolling_stone_critic)
```

# [Presentación Gráfica de los Resultados Anteriores]

# [Conclusiones: Interpretaciones Básicas del Análisis Exploratorio]


# Análisis multidimensional variables numéricas
